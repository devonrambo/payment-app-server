...
public class JdbcUserDAO implements UserDAO {
    ...
    @Override
    public int findAccountIdWithUserId(int userId) {
        String sql = "SELECT account_id FROM accounts WHERE user_id = ?";
        SqlRowSet result = jdbcTemplate.queryForRowSet(sql, userId);
        int returnId = 0;

        if (result.next()) {
            returnId = result.getInt("account_id");
        }
    ...
    @Override
    public int findUserIdWithAccountId(int accountId) {
        ...
        if (result.next()) {
            returnId = result.getInt("user_id");
        }
    ...
    public String findUsernameByUserId(int userId) {
        ...
        if(result.next()){
            ...
            returnString = result.getString("username");
